...
/**
  ...
  */
trait ConfigurationZookeeperClient {
    /**
...
/**
  * Constructs and deconstructs [[ConfigurationZookeeperClient]] values.
  *
  * A configuration is constructed by first specifying ''required'' attributes via `Configuration()` and then attaching
  * ''optional'' attributes as necessary. A set of implicit methods conveniently convert between instances of [[ConfigurationZookeeperClient]]
  * and [[ConfigurationZookeeperClient.Builder Builder]].
  *
  * Example:
  * {{{
  ...
  * }}}
  *
  * The type of `config` above is [[ConfigurationZookeeperClient.Builder Builder]] since an implicit conversion occurred when attaching
  * optional attributes using the various `with` methods. An explicit conversion back to [[ConfigurationZookeeperClient]], which can be
  * accomplished using `build()`, is unnecessary since another implicit will perform this function automatically.
  */
object ConfigurationZookeeperClient {
    /**
      ...
      */
    def apply(servers: Seq[InetSocketAddress]): ConfigurationZookeeperClient = new Builder(servers) build
    ...
    /**
      ...
      */
    def unapply(config: ConfigurationZookeeperClient): Option[(Seq[InetSocketAddress], String, Duration, (StateEvent, Session) => Unit, Boolean, ExecutionContext)] = {
        if (config == null)
            None
        else
    ...
    private class ImplConfigurationZookeeperClient(
                        val servers: Seq[InetSocketAddress],
                        val path: String,
                        val timeout: Duration,
                        val watcher: (StateEvent, Session) => Unit,
                        val allowReadOnly: Boolean,
                        val exec: ExecutionContext) extends ConfigurationZookeeperClient
    ...
    class Builder private(
        ...
        private[ConfigurationZookeeperClient] def this(servers: Seq[InetSocketAddress]) {
            this(servers, "", 60 seconds, null, false, null)
        }

        private[ConfigurationZookeeperClient] def this(config: ConfigurationZookeeperClient) {
            this(config.servers, config.path, config.timeout, config.watcher, config.allowReadOnly, config.exec)
        }

        def withPath(path: String): Builder = new Builder(servers, path, timeout, watcher, allowReadOnly, exec)

        def withTimeout(timeout: Duration): Builder = new Builder(servers, path, timeout, watcher, allowReadOnly, exec)

        def withWatcher(watcher: (StateEvent, Session) => Unit): Builder = new Builder(servers, path, timeout, watcher, allowReadOnly, exec)

        def withAllowReadOnly(allowReadOnly: Boolean): Builder = new Builder(servers, path, timeout, watcher, allowReadOnly, exec)

        def withExec(exec: ExecutionContext): Builder = new Builder(servers, path, timeout, watcher, allowReadOnly, exec)

        def build(): ConfigurationZookeeperClient = new ImplConfigurationZookeeperClient(servers, path, timeout, watcher, allowReadOnly, exec)
    }

    implicit def configToBuilder(config: ConfigurationZookeeperClient): Builder = new Builder(config)
    implicit def builderToConfig(builder: Builder): ConfigurationZookeeperClient = builder.build
}
